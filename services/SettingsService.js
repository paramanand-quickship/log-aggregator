	...
	async save (updates) {
		...
		if (updates.retentionDays !== undefined) {
			...
			if (isNaN(v) || v < 1 || v > 365) {
				throw Object.assign(new Error('retentionDays must be 1–365'), {
					status: 400,
				});
			}
		...
		if (updates.webhookUrl !== undefined) {
			...
			if (url && !/^https?:\/\/.+/.test(url)) {
				throw Object.assign(
					new Error('webhookUrl must be a valid http(s) URL or empty'),
					{ status: 400 },
				);
			}
			next.webhookUrl = url;
		...
		if (next.webhookUrl !== undefined) {
			config.WEBHOOK_URL = next.webhookUrl;
		}
		if (next.enableStream !== undefined) {
