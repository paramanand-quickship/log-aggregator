	...
	ingest (rawLogs, defaultAppName = 'unknown') {
		...
		for (let i = 0; i < rawLogs.length; i++) {
			...
			if (raw.length > 64_000) {
				throw Object.assign(new RangeError(`logs[${i}] exceeds 64 KB`), {
					status: 400,
				});
			}
   ...
   */
	...
	async _resolveFiles (service, date) {
		...
		if (service && date) {
			...
			files.push({
				appName: service,
				filePath: safeLogPath(config.LOG_BASE_DIR, service, date),
			});
		} else if (service) {
			try {
				sanitizeAppName(service);
			...
			for (const { appName } of services) {
				try {
					...
					files.push({
						appName,
						filePath: safeLogPath(config.LOG_BASE_DIR, appName, targetDate),
					});
				} catch {}
			}
		}
