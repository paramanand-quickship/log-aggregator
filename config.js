...
if (process.env.NODE_ENV === 'production') {
	...
	if (!API_KEY || API_KEY === 'change-me-secret-key') {
		throw new Error('[Config] Set a real API_KEY in production');
	}
}
...
module.exports = {
	...
	LOG_BASE_DIR: path.resolve(
		envStr('LOG_BASE_DIR', path.join(__dirname, 'logs')),
	),
	...
	CORS_ORIGINS: envStr('CORS_ORIGINS', 'http://localhost:9900')
		.split(',')
		.map((s) => s.trim()),
	USERS_FILE: path.resolve(
		envStr('USERS_FILE', path.join(__dirname, 'users.json')),
	),
	SETTINGS_FILE: path.resolve(
		envStr('SETTINGS_FILE', path.join(__dirname, 'settings.json')),
	),
