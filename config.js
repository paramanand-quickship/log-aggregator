...
if (process.env.NODE_ENV === 'production') {
	if (!JWT_SECRET || JWT_SECRET.length < 32) { throw new Error('[Config] JWT_SECRET must be at least 32 characters in production'); }
	if (!API_KEY || API_KEY === 'replace-with-a-secure-api-key') { throw new Error('[Config] API_KEY must be set to a real value in production'); }
}
