...
class BatchWriter {
  ...
  write(appName, date, log) {
    ...
    if (!this.buffer.has(key)) this.buffer.set(key, { appName, date, logs: [] });
    const entry = this.buffer.get(key);
    entry.logs.push(log);
    if (entry.logs.length >= config.BATCH_SIZE) {
      this._flushKey(key);
    } else if (!this.timers.has(key)) {
      this.timers.set(key, setTimeout(() => this._flushKey(key), config.BATCH_TIMEOUT));
    }
  }
  ...
  _flushKey(key) {
    ...
    for (const log of logs) {
      stream.write(`${log}\n`, err => { if (err) logger.error(`[BatchWriter] ${appName}/${date}: ${err.message}`); });
    }
  }

  async flushAll() { for (const key of this.buffer.keys()) this._flushKey(key); }
}
