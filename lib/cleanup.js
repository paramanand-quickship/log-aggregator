...
async function cleanupOldLogs () {
	...
	let appDirs;
	try { appDirs = await fs.readdir(config.LOG_BASE_DIR); } catch (err) { if (err.code === 'ENOENT') { return; } throw err; }
	...
	for (const app of appDirs) {
		...
		try {
			...
			for (const file of files) {
				...
				try { await fs.unlink(path.join(appPath, file)); deleted++; logger.info(`[Cleanup] Deleted ${app}/${file}`); } catch (e) { logger.error(`[Cleanup] Failed to delete ${app}/${file}: ${e.message}`); }
			}
